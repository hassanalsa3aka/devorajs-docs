// Shared by the landing page's "How it compares" table and /compare, so
// both tables look identical and every brand mark lives in one place.
...
export const LOGO_DEVORA = <img className="home-compare-logo home-compare-logo-devora" src="/icons/devorajs-logo-withoutbg.png" alt="" />;
export const LOGO_REACT = (
...
export const LOGO_REACT_ROUTER = (
  <svg className="home-compare-logo logo-react-router" viewBox="0 0 24 24" aria-hidden="true">
...
export const LOGO_SVELTE = (
  <svg className="home-compare-logo logo-svelte" viewBox="0 0 24 24" aria-hidden="true">
...
export const LOGO_ASTRO = (
  <svg className="home-compare-logo logo-astro" viewBox="0 0 24 24" aria-hidden="true">
...
export const LOGO_EXPRESS = (
  <svg className="home-compare-logo logo-express" viewBox="0 0 24 24" aria-hidden="true">
...
export const LOGO_FASTIFY = (
  <svg className="home-compare-logo logo-fastify" viewBox="0 0 24 24" aria-hidden="true">
...
export const LOGO_NEST = (
  <svg className="home-compare-logo logo-nest" viewBox="0 0 24 24" aria-hidden="true">
...
// Styles for <CompareTable> — rendered once per page inside a <style> tag.
export const COMPARE_TABLE_CSS = `
.compare-table-wrap {
...
.compare-table thead .compare-table-col .home-compare-logo {
  width: 24px;
  height: 24px;
  margin-right: 0;
  vertical-align: unset;
}
.compare-table thead .compare-table-col .home-compare-logo-group { margin-right: 0; }
/* Every logo sits in the same fixed-height slot, so the column labels
   under them line up regardless of each mark's optical size below. */
.compare-table-logo-slot { height: 30px; display: flex; align-items: center; justify-content: center; }
...
.compare-table thead .compare-table-col .logo-react { width: 28px; height: 28px; }
.compare-table thead .compare-table-col .logo-turbo,
.compare-table thead .compare-table-col .logo-nx { width: 22px; height: 22px; }
.compare-table thead .compare-table-col .home-compare-logo-devora { width: 30px; height: 30px; }
.compare-table thead .compare-table-col .logo-express { width: 28px; height: 28px; }
.compare-table thead .compare-table-col .logo-react-router { width: 27px; height: 27px; }
.compare-table thead .compare-table-col .logo-svelte,
.compare-table thead .compare-table-col .logo-nest { width: 23px; height: 23px; }
.compare-table thead .compare-table-col .logo-astro,
.compare-table thead .compare-table-col .logo-fastify { width: 24px; height: 24px; }
...
.compare-table tbody td { color: var(--devora-fg-muted); line-height: 1.45; }
...
// columns: [{ label, logo, highlight? }] — logo is one mark or an array
// (rendered as a group, e.g. Turborepo + Nx). rows: [{ label, cells }] with
// one cell per column, in column order.
export function CompareTable({ columns, rows, minWidth = 800, className = "" }) {
  return (
    <div className={"compare-table-wrap " + className}>
      <table className="compare-table" style={{ minWidth }}>
        <thead>
          <tr>
            <th scope="col" />
            {columns.map((col) => (
              <th scope="col" key={col.label} className={col.highlight ? "compare-table-highlight" : undefined}>
                <span className="compare-table-col">
                  <span className="compare-table-logo-slot">
                    {Array.isArray(col.logo) ? (
                      <span className="home-compare-logo-group">{col.logo}</span>
                    ) : (
                      col.logo
                    )}
                  </span>
          ...
          {rows.map((row, r) => (
            <tr key={r}>
              ...
              {row.cells.map((cell, i) => (
                <td key={i} className={columns[i].highlight ? "compare-table-highlight" : undefined}>
                  {cell}
                </td>
              ))}
            </tr>
